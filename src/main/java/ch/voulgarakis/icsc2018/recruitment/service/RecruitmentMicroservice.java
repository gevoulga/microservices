...
public class RecruitmentMicroservice implements RecruitmentService {
    ...
    private void saveSkills(Collection<String> skills) {
        // The REST requests to store all the skills defined in the Applicant
        skills.stream().forEach(skill -> {
            rest.postForEntity("http://skill-service/skill", new HttpEntity<>(new Skill(skill)), Skill.class);
            // System.out.println("Skill List: " + rest.exchange("http://skill-service/skill", HttpMethod.GET, null,
    ...
    public Applicant saveApplicant(Applicant applicant) {
        ...
        return postResponse.getBody() == null ? loadApplicant(applicant.getName()) : postResponse.getBody();
    }
    ...
    public Vacancy saveVacancy(Vacancy vacancy) {
        ...
        // Return the response body, which contains the applicants
        return postResponse.getBody() == null ? loadVacancy(vacancy.getName()) : postResponse.getBody();
    }
    ...
    public double apply(Applicant applicant, Vacancy vacancy) {
        List<String> sV = vacancy.getRequiredSkills();
        List<Double> wV = vacancy.getRequiredSkillWeights();

        List<String> sA = applicant.getSkillSet();
        ...
        Application application = new Application(applicant.getId(), vacancy.getId(), fitRatio);
